...
func TestContext2Apply_provisionerInterpCount(t *testing.T) {
	...
	dir, err := ioutil.TempDir("", "tf-test-interp-count")
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(dir)

	planFilename := filepath.Join(dir, "tfplan")
	var planBuf bytes.Buffer
	err = planfile.Create(planFilename, snap, stateFile, plan)
	if err != nil {
		t.Fatalf("failed to write plan: %s", err)
	}
	planFile, err := planfile.Open(planFilename)
	plan, err = planFile.ReadPlan()
	if err != nil {
		t.Fatalf("failed to read plan: %s", err)
	}
	snap, err = planFile.ReadConfigSnapshot()
	if err != nil {
		t.Fatalf("failed to read config snapshot: %s", err)
	}
	stateFile, err = planFile.ReadStateFile()
	if err != nil {
	...
	if configDiags.HasErrors() {
		t.Fatalf("failed to reload config from snapshot: %s", configDiags.Error())
	}
